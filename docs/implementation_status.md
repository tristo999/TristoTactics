...
| 2 — TUTORIAL (drill → breach → raid, ONE scene) | First tactical combat as one continuous scene: a coached **drill** (5 lessons on the yard's own features) → the north fence **breaches mid-drill** (3 lanes) → the **raid** on the same ground, no cut. Hero-only control; squad scripted. **Mirror:** Act 2 B2 is inside this camp during the raid. | `data/maps/training_grounds.map` (`gen_training_grounds.py`); design `docs/levels/training_grounds.md` | `training_grounds_scene.tscn` + `training_grounds.gd` | 🟡 | **Built 2026-06-10** on the TriggerEngine + a sequential drill director: lessons MOVE → STRIKE (Borin, parked willing target) → **COVER** (+2 DEF taught live, first map to use it) → **TOGETHER** (Elena's follow-up chains live, honest branch) → **MEND** (Lyra arrives, scripted heal) → breach: `flash` → `spawn` 3 raid groups (+`register_units`, new GM API) → **Borin `set_team` flips live** → all AI unparks → battle music's first entrance → raid → victory coda. Camera: WALK 2.0 → BATTLE 4.0 handover ease. **Machine-certified 2026-06-11**: the level-1 autoplay harness (`scenes/dev/level1_autoplay.tscn`) plays drill→breach→raid end to end — VICTORY, 67 player turns; 9 bugs found+fixed in the process (see commit f8bb367). Still needs a HUMAN playthrough for feel/pacing; balance note: the AI squad can win the raid without the hero engaging. Old `tutorial_spar_scene`/`tutorial_battle_scene` + `arena_drill/raid` maps SUPERSEDED (kept for reference). |
| 3 — EARLY CAMPAIGN | Missions against "insurgents", subtle anomalies begin | Kingdom outskirts (battle) | — | ⬜ | **Scoped 2026-06-10** — see `docs/levels/outskirts_road.md` (PROPOSAL): one mission on the TriggerEngine (wave spawn, first glitch, moral flicker, cue-language plant) + **bonding v1** (bonds-through-use accrual). Awaiting keep/kill on the `[GUESS]` fiction/lines |
...
1. **Companion identities** (names, origins, wounds). See the world bible's open questions. Blocks Beats 2–10 dialogue and Beat 7 "The Choice" mechanic. Cores designed (sixth batch, 2026-06-12): Archer, Healer, Tank/dwarf, Warlock, Rogue, Paladin, Knight — only the Wizard's wound remains a proposal. Roster settled at eight in role pairs; gender grid 4/4 (bible_02).
